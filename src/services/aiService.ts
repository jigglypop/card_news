...
JSON 형식으로만 응답해주세요.`;
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful AI assistant that specializes in analyzing IT news articles and extracting key information. Respond only with the requested JSON format.'
            },
      ...
      '뉴스 분석 중 오류 발생',
      this.createBasicAnalysis(newsItem)
    );
  }
  ...
  public async generateCardNewsContent(
...
2. 뉴스 슬라이드 (각 주요 뉴스마다):
- 제목: 15자 이내의 간결하고 명확한 제목 (제목이 길면 반드시 15자 이내로 요약해주세요)
- 내용: 아래 지침에 따른 포인트식 뉴스 요약
  * 뉴스 내용은 2-3문장으로 나누고, 각 문장 사이에 줄바꿈 처리
  * 각 문장은 20자 이내로 작성하여 간결하게
  * 명사형으로 끝맺고 "~입니다" 등의 종결어미는 사용하지 않음
  * 핵심 키워드는 글자에 강조 표시(볼드체는 넣지 않습니다)
  * 전체 내용은 80자 이내로 제한
- 태그: 관련 키워드 (정확히 3개만 선택)
- 출처 URL: 원문 링크 (반드시 포함하며, 출처 링크를 간결하게 작성)

3. 마무리 슬라이드:
- 타입: news (summary가 아닌 news 타입을 사용하세요)
- 제목: "오늘의 핵심 트렌드"
- 내용: 전체 트렌드를 2-3문장으로 요약 (각 문장은 20자 이내, 문장 사이 줄바꿈 처리)
- 태그: ["IT", "트렌드", "혁신"] (반드시 이 태그 사용)
...
JSON 형식으로 응답해주세요. 각 슬라이드는 다음 구조를 가져야 합니다:
{
  "slides": [
    ...
    ...
    {
      ...
      "tags": ["IT", "트렌드", "혁신"]
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful AI assistant that specializes in creating engaging card news content from IT news articles. Respond only with the requested JSON format.'
            },
            { role: 'user', content: prompt }
          ],
          response_format: { type: 'json_object' }
        ...
        try {
          ...
          const logPath = path.join(
            process.cwd(),
            'logs',
            `cardnews-content-${new Date().toISOString().replace(/:/g, '-')}.json`
          );
      ...
      '카드뉴스 생성 중 오류 발생',
      JSON.stringify({ slides: this.createBasicSlides(analyzedNews, frequency) })
    );
  }
  ...
  private createBasicSlides(analyzedNews: AnalyzedNews[], frequency: 'daily' | 'monthly'): any[] {
    ...
    // 마지막 요약 슬라이드 - 'news' 타입으로 변경해 동일한 템플릿 사용
    slides.push({
      type: 'news',
      title: '오늘의 핵심 트렌드',
      content: '기술 혁신과 디지털 전환이\n지속적으로 진행 중',
