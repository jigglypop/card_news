...
router.get('/card-news', (_, res) => {
  return tryCatch(async () => {
    ...
    res.json(cardNews);
  }, '카드뉴스 목록 조회 중 오류 발생', res).catch(error => {
    res.status(500).json({
      success: false,
...
router.get('/test-news', async (req, res) => {
  try {
    ...
    const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=kr&category=technology&pageSize=5&apiKey=${apiKey}`);
    ...
    return res.json({
      ...
      sampleArticle: response.data.articles?.length > 0 ? {
